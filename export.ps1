# export.ps1 — Static export for GitHub Pages (audiobook-maker)
# Builds the SvelteKit app with adapter-static into docs/ so it can be served
# from https://Mtothexmax.github.io/audiobook-maker/.
#
...
param(
    [string]$BasePath = "/audiobook-maker"
)

$ErrorActionPreference = 'Stop'

Write-Host "=== audiobook-maker Export fuer GitHub Pages ===" -ForegroundColor Cyan
...
try {
    ...
    if (Test-Path 'docs') {
        $size = (Get-ChildItem 'docs' -Recurse -File | Measure-Object -Property Length -Sum).Sum / 1MB
        Write-Host "Deployed to docs/ ($([math]::Round($size, 1)) MB)" -ForegroundColor Green
    }
} finally {
